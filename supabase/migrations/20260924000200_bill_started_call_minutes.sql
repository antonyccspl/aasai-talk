...
-- Calls are prepaid by started minute: debit minute 1 as soon as media is
-- connected, then debit minute 2 at 01:00, minute 3 at 02:00, and so on.
...
begin
  ...
  if call_row.id is null or call_row.status <> 'connected' or call_row.connected_at is null then
    return jsonb_build_object('charged', false, 'call_active', false,
      'minute_complete', false, 'remaining_coins', coalesce(wallet.coins, 0));
  end if;

  -- Do not permit prepaying future minutes. At 00:00 minute 1 is payable;
  -- at 01:00 minute 2 becomes payable.
  if input_minute_number > floor(extract(epoch from now() - call_row.connected_at) / 60)::integer + 1 then
  ...
  select greatest(0,
    coalesce(sum(l.duration_seconds), 0) + coalesce((
