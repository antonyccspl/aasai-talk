...
drop function if exists public.clear_phone_call_pair(text, text);
...
begin
  ...
  -- A retry from the caller supersedes an unanswered ringing invitation.
  update public.call_sessions
  set status = 'cancelled', ended_at = now(), duration_seconds = 0
