...
-- Keep message earnings separate from call earnings so every host credit is
-- traceable and a retry can never pay the same message twice.
create table if not exists public.phone_message_host_earnings_ledger (
...
create index if not exists phone_message_host_earnings_ledger_host_created_idx
  on public.phone_message_host_earnings_ledger(host_phone, created_at desc);
alter table public.phone_message_host_earnings_ledger enable row level security;
revoke all on public.phone_message_host_earnings_ledger from public, anon, authenticated;
grant all on public.phone_message_host_earnings_ledger to service_role;
...
declare
  ...
  recipient_is_approved_host boolean;
...
begin
  ...
  select exists(
    ...
    where phone = input_recipient_phone and status = 'approved'
  ) into recipient_is_approved_host;

  if recipient_is_approved_host then
    update public.phone_wallets
...
begin
  ...
  return jsonb_build_object(
    ...
    'today_earnings_paise', coalesce((select sum(earnings_paise) from public.phone_host_earnings_ledger where host_phone=input_phone and created_at>=current_date),0) + coalesce((select sum(earnings_paise) from public.phone_message_host_earnings_ledger where host_phone=input_phone and created_at>=current_date),0),
    'total_calls', (select count(*) from public.host_call_time_ledger where host_phone=input_phone),
    'total_earnings_paise', coalesce((select sum(earnings_paise) from public.phone_host_earnings_ledger where host_phone=input_phone),0) + coalesce((select sum(earnings_paise) from public.phone_message_host_earnings_ledger where host_phone=input_phone),0),
    'message_earning_paise', 35,
...
$$;
revoke all on function public.send_phone_message(text, text, text) from public, anon, authenticated;
grant execute on function public.send_phone_message(text, text, text) to anon, authenticated;
revoke all on function public.get_phone_host_dashboard(text) from public, anon, authenticated;
