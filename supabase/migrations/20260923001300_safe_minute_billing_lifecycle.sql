...
begin
  ...
  if wallet.coins < coin_cost then
    return jsonb_build_object('charged', false, 'call_active', true,
      'insufficient_balance', true, 'diamonds_required', diamonds,
      'coins_required', coin_cost, 'remaining_coins', wallet.coins);
  end if;
  ...
  return jsonb_build_object('charged', true, 'already_charged', false,
    'call_active', true, 'diamonds_charged', diamonds,
    'coins_charged', coin_cost, 'remaining_coins', wallet.coins - coin_cost);
end;
$$;
