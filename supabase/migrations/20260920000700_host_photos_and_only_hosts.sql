...
create policy host_photos_public_read on storage.objects
  for select to anon, authenticated using (bucket_id = 'host-photos');
...
create policy host_photos_public_upload on storage.objects
  for insert to anon, authenticated with check (bucket_id = 'host-photos');
...
create policy host_photos_public_update on storage.objects
  ...
  using (bucket_id = 'host-photos') with check (bucket_id = 'host-photos');
...
begin
  if input_phone is null or input_phone !~ '^\+91[0-9]{10}$'
     ...
     or profile_name is null or profile_username is null
  ...
  insert into public.phone_profiles
    (phone, display_name, username, bio, gender, date_of_birth, city, languages, interests, avatar_url)
  values
    (input_phone, profile_name, profile_username, coalesce(input_profile ->> 'bio', ''),
     ...
     array(select jsonb_array_elements_text(input_profile -> 'interests')), profile_photo)
...
returns table (id text, display_name text, age smallint, gender text, city text,
  languages text[], interests text[], bio text, availability text,
  avatar_url text, accent_color text)
language sql security definer set search_path = ''
as $$
  select 'phone_' || replace(replace(replace(pp.phone, '+', ''), ' ', ''), '-', ''),
    pp.display_name, extract(year from age(current_date, pp.date_of_birth))::smallint,
    pp.gender, pp.city, pp.languages, pp.interests, pp.bio, 'Available',
    pp.avatar_url, '#285647'
  from public.phone_profiles pp
  join public.host_applications ha on ha.phone = pp.phone
  where ha.status = 'approved' and pp.gender = 'Female'
