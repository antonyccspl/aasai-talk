...
declare
  ...
  diamonds_charged numeric;
  minutes integer;
  coins_per_minute bigint;
  charge bigint;
...
begin
  ...
  minutes := case
    when call_row.duration_seconds > 0
      then ceil(call_row.duration_seconds::numeric / 60)::integer
    else 0
  end;
  diamonds_charged := minutes * diamonds_per_minute;
  coins_per_minute := ceil(slab.diamonds_per_minute * slab.coins_per_diamond)::bigint;
  charge := minutes * coins_per_minute;
  ...
  return jsonb_build_object(
    ...
    'diamonds_charged', diamonds_charged,
    'diamonds_per_minute', diamonds_per_minute,
