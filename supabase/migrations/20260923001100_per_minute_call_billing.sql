...
create table if not exists public.phone_call_minute_ledger (
  ...
  unique(phone, call_session_id, minute_number)
);

alter table public.phone_call_minute_ledger enable row level security;
revoke all on public.phone_call_minute_ledger from public, anon, authenticated;
grant all on public.phone_call_minute_ledger to service_role;
...
declare
  ...
  coin_cost bigint;
begin
  if input_minute_number < 1 then raise exception 'Invalid call minute'; end if;
  ...
  if not found then raise exception 'Call is not connected'; end if;
  if floor(extract(epoch from now() - call_row.connected_at) / 60)::integer < input_minute_number
    then raise exception 'Call minute is not complete'; end if;
  ...
  if wallet.coins < coin_cost then
    return jsonb_build_object('charged', false, 'insufficient_balance', true,
      'diamonds_required', diamonds, 'coins_required', coin_cost,
      'remaining_coins', wallet.coins);
  end if;

  update public.phone_wallets set coins = phone_wallets.coins - coin_cost, updated_at = now()
  where phone = input_phone;
  update public.phone_wallets set earnings_paise = earnings_paise + coin_cost * 100, updated_at = now()
  where phone = call_row.host_phone;
  insert into public.phone_call_minute_ledger(phone, call_session_id, minute_number,
    diamonds_charged, coin_delta)
