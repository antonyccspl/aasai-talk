...
begin
  ...
  if exists (select 1 from public.phone_wallet_ledger
    where phone = call_row.caller_phone and call_session_id = input_session_id) then
  ...
  update public.phone_wallets set coins = phone_wallets.coins - charge, updated_at = now()
  where phone = call_row.caller_phone;
  update public.phone_wallets set earnings_paise = earnings_paise + host_earnings, updated_at = now()
